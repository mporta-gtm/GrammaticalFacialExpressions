...
class UnfoldTemporalWindows(nn.Module):
    ...
    def forward(self, x):
        ...
        N, C, T, V = x.shape
        x = self.unfold(x) # N, C*window_size, L=T*V
        # Permute extra channels from window size to the graph dimension; -1 for number of windows
        x = x.view(N, C, self.window_size, -1, V).permute(0,1,3,2,4).contiguous() # N,C, window_size, L/V=T, V -> N, C, L/V=T, window_size, V
        x = x.view(N, C, -1, self.window_size * V) # N, C, L/V=T, window_size*V
        return x
...
class SpatialTemporal_MS_GCN(nn.Module):
    ...
    def forward(self, x):
        N, C, T, V = x.shape    # L/V=T = number of windows
